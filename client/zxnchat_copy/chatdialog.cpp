    ...
    _state(ChatUIMode::ChatMode),_last_widget(nullptr),_cur_chat_uid(0)
{
    ...
    //连接好友信息界面发送的点击事件
    connect(ui->friends_info_page, &FriendInfoPage::sig_jump_chat_item, this,
...
void ChatDialog::slot_friend_info_page(std::shared_ptr<UserInfo> user_info)
{
    ...
    _last_widget = ui->friends_info_page;
    ui->stackedWidget->setCurrentWidget(ui->friends_info_page);
    ui->friends_info_page->SetInfo(user_info);
}
